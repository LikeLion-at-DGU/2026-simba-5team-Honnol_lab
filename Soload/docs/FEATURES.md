...
| 프로필 | 프로필 수정 | 닉네임·프로필 사진 변경(사진 없으면 레벨 병아리) | edit_profile | Profile |
...
| 후기 | 작성/수정/삭제 (CRUD) | 항목+사진 업로드, 작성 시 경험치/레벨업 | createreview / review_update / review_delete | Review, ReviewImage |
| 후기 | 좋아요 | 후기 좋아요 토글(M:N) | review_like | Review, User |
| 후기 | 장소 정보 자동보정 | 후기 평균·다수결로 Place 추천레벨/시설 갱신 | createreview | Place, Review |
| 찜 | 토글 | 중복방지(unique_together), fetch 토글 | place_like | PlaceLike |
| 소셜 | 팔로우/팔로잉 | 유저 팔로우 토글(M:N self), 팔로워/팔로잉 목록 | follow | Profile |
| 마이/프로필 | 내·타 유저 페이지 | 내 찜·후기 / 타 유저 프로필·후기 | mypage / user_profile | PlaceLike, Review |